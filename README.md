...
- **공공기관 노인 케어 음성 비서** — 1인 용역으로 백엔드·런타임·연동 중심 개발 담당, 2026.05 납품 목표로 진행 중
...
- ※ 수치형 성과(메모리 절감, mAP, 테스트 케이스)는 프로젝트 수행 중 기록한 내부 측정/실험 결과이며, 세부 로그와 조건은 별도 문서로 정리 중입니다.
...
| 기간 | 역할 | 상태 |
|------|------|------|
| 2025.02 ~ 현재 | 1인 용역 — 백엔드, 대시보드, Jetson 런타임, ESP32 오디오 펌웨어 수정·튜닝, 케어센터 API 연동 | 2026.05 납품 목표로 진행 중 |
...
- **측정 메모** — 메모리 수치는 프로젝트 수행 중 내부 기록 기준이며, 세부 측정 조건은 문서화 진행 중
...
**Stack:** Kneron Toolchain · RKNN Toolkit · ONNX · Netron · Docker
...
| 기간 | 결과 | 카테고리 |
|------|------|----------|
| 2025.02 ~ 2025.09 | mAP@0.5 84.7% (내부 실험 기록) | Vision · Model Training |
...
- 토큰 기반 인증 구현
- AI 에이전트를 활용해 서버/도구/파이프/유틸 모듈 대상 53개 테스트 케이스 생성 (내부 테스트 기록 기준)
